...
public class TransportShiftTimeWindowDuration {
    ...
    public static Feature createFeature(String json) throws ParseException {
        ...
        for (int k = 0; k < jsonObj2.size(); k++) {
            ...
            for (int i = 0; i < jsonObj4.size(); i++) {
                ...
                result += (long) jsonObject7.get("timeTo") - (long) jsonObject7.get("timeFrom");
                count++;
            }
        }
        return new Feature("AverageTransportTimeWindowDuration", result / count, "Среднее временное окно действия смены, в минутах.Продолжительность окна.");
    }
}

...
public class CountCapacityWindowTypePICKUPFeature {
    ...
    public static Feature createFeature(String json) throws ParseException {
        ...
        for (int i = 0; i < jsonObj2.size(); i++) {
            ...
            for (int j = 0; j < jsonObj4.size(); j++) {
                ...
                if (result.equals("CWT_PICKUP")) {
                    count++;
                }
            }
        }
        return new Feature("CapacityWindowTypePICKUP", count, "Количество окон на загрузку (перемещение груза из локации в транспорт).");
    }

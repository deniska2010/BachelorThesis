...
public class CapacityTimeWindowToFeature {
    ...
    public static Feature createFeature(String json) throws ParseException {
        ...
        if (count == 0) {
            return new Feature("AverageCapacityTimeWindowTo", count, "Окна загрузки/разгрузки (CapacityWindow) для локаций определяют количество груза, которое локация способна отдать\\принять. Cреднее начало временного окна по всем локациям.");
        }

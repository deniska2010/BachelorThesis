 ...
 */
public class AverageEventHardTimeWindowFromFeature {
        ...
        for (int k = 0; k < jsonObj2.size(); k++) {
            ...
            for (int i = 0; i < jsonObj4.size(); i++) {
                ...
                for(int j = 0; j < jsonObj6.size();j++){
                    ...
                    JSONObject jsonObject8 = (JSONObject) jsonObject7.get("hardTimeWindow");
                    result += (long) jsonObject8.get("timeFrom");
                    count++;
                }
            }
        }
        return new Feature("AverageEventHardTimeWindowFrom", result / count, "Среднее жесткое временное окно для события.Конец окна.");
    }
}

...
public class AverageEventHardTimeWindowToFeature {
        ...
        for (int k = 0; k < jsonObj2.size(); k++) {
            ...
            for (int i = 0; i < jsonObj4.size(); i++) {
                ...
                for(int j = 0; j < jsonObj6.size();j++){
                    ...
                    result += (long) jsonObject8.get("timeTo");
                    count++;
                }
            }
        }
        return new Feature("AverageEventHardTimeWindowTo", result / count, "Среднее жесткое временное окно для события.Начало окна.");
    }
}

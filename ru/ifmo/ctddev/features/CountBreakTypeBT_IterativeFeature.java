...
public class CountBreakTypeBT_IterativeFeature {
        ...
        for (int k = 0; k < jsonObj2.size(); k++) {
            ...
            for (int i = 0; i < jsonObj4.size(); i++) {
                ...
                for (int j = 0; j < jsonObj6.size(); j++) {
                    ...
                    if (result.equals("BT_ITERATIVE")) {
                        count++;
                    }
                }
            }
        }
        return new Feature("NumberOfBreakTypeBT_ITERATIVE", count, "Количество перерывов,которые необходимо выполнять раз в указанное количество минут (break_time), при этом считается время работы driving_time + waiting_time.");
    }
}
